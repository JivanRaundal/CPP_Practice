...
void operator>>(istream & din, Num & T)
{
	cout<<"\nEnter x & y : ";
	din>>T.x>>T.y;	//OR cin>>T.x>>T.y; is also valid
}

void operator<<(ostream & dout, Num T)
{
...
int main()
{
	...
	cin>>T1;
	cout<<T1;
	/* Note- In this code, overloaded << & >> functions will work for single object only.
	         & not for cin>>T1>>T2>>T3; or cout<<T1<<T2<<T3;
	         i.e. CASCADING IS NOT POSSIBLE HERE */
	//cin>>T1>>T2>>T3;	//error
	//cout<<T1<<T2<<T3;	//error
	return 0;
}
